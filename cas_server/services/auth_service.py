...
def _commit_or_conflict(
    ...
    *,
    ...
    """Runs `action` (defaults to session.commit), translating a
    unique-constraint IntegrityError (a concurrent CreateUser call for the
    same username that slipped past this request's own pre-check SELECT)
    into ALREADY_EXISTS instead of an uncaught INTERNAL/UNKNOWN. Mirrors
    common.py's confirmar_o_duplicado -- kept as a local, English-named
    equivalent since this module predates that extraction and doesn't
    otherwise import from common.py (see CLAUDE.md's note on the language
    split).

    Pass action=session.flush when the caller needs an explicit flush()
    before commit() (e.g. to get an autogenerated id for an AuditLog row) --
    an INSERT with a unique-constraint conflict fails at that flush(), not
    at the later commit(), so wrapping only the final commit isn't enough
    in that case. See ES-006 §3.1."""
    action = action or session.commit
    try:
        action()
    except IntegrityError:
        session.rollback()
        context.abort(grpc.StatusCode.ALREADY_EXISTS, conflict_message)
